            ...
            <div class="intro-y inbox box mt-5">
                <div class="p-5 flex flex-col-reverse sm:flex-row text-gray-600 border-b border-gray-200 dark:border-dark-1">
                    <div class="flex items-center mt-3 sm:mt-0 border-t sm:border-0 border-gray-200 pt-5 sm:pt-0 mt-5 sm:mt-0 -mx-5 sm:mx-0 px-5 sm:px-0">
                        ...
                        <a href="javascript:;" class="w-5 h-5 ml-5 flex items-center justify-center dark:text-gray-300 restore-all" title="@translate(Restore selected email)">
                            <x-feathericon-rotate-ccw/>
                        </a>
                        
                        <a href="javascript:;" class="w-5 h-5 ml-5 flex items-center justify-center dark:text-gray-300 destory-all" title="@translate(Permanent delete selected email)">
                            <x-feathericon-trash-2/>
                        </a>
                    </div>
                    <div class="flex items-center sm:ml-auto">

                        <div class="dark:text-gray-300 ml-5">@translate(Total) {{ number_format(trashedCount()) }} @translate(email)</div>
                            ...
                            <div class="cursor-pointer inline-block sm:block text-gray-700 dark:text-gray-500 bg-gray-100 dark:bg-dark-1 border-b border-gray-200 dark:border-dark-1">
                                <div class="flex px-5 py-3">
                                    <div class="w-56 flex-none flex items-center mr-10">
                                        <input class="input flex-none border border-gray-500 checking" data-id="{{ $trash->id }}" name="check" type="checkbox">
                                        <div class="w-6 h-6 flex-none image-fit relative ml-5">
                                            <img alt="{{ $trash->email }}" class="rounded-full" src="{{ emailAvatar($trash->email) }}">
                                        </div>

                                        <a href="{{ route('email.contact.show', $trash->id) }}" class="w-5 h-5 flex-none ml-4 flex items-center justify-center text-gray-500">
                                            <x-feathericon-edit/>
                                        </a>
                                    ...
                                    <div class="grid grid-cols-3 w-full gap-4">
                                        ...
                                        <span class="inbox__item--highlight">{{ $trash->email }}</span>
                                    </div>

                                    <div class="w-64 sm:w-auto truncate mr-10">
                                        <span class="inbox__item--highlight">{{ $trash->name }}</span>
                                    </div>

                                    <div class="w-64 sm:w-auto truncate mr-10">
                                        <span class="inbox__item--highlight">{{ $trash->phone }}</span>
                                    </div>
                                    </div>

                                    
                                    <div class="inbox__item--time whitespace-no-wrap ml-auto pl-10">{{ $trash->created_at->format('H:i a') }}</div>
                                </div>
                            </div>
                        </div>
                    @empty
                ...
                <div class="p-5 flex flex-col sm:flex-row items-center text-center sm:text-left text-gray-600">
                    <div class="dark:text-gray-300">@translate(Total) {{ number_format(trashedCount()) }} @translate(email)</div>
                </div>
            </div>
            <!-- END: Inbox Content -->
